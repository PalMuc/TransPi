...
## FIXME: wasp.simple.tab has excess columns for groupname1..9 << spaces or tabs confused, only 1 col
...
# unused: my $jTEscore= 10; #  index from scorevec in gene.gff  # FIXME, read from .gff:#scoretype
# unused: my @VKEY= qw(ID osrc oid gene alttr aalen cxlen nintron join must scorevec);
...
my @ATKEY= ( qw(ID gene isoform quality aaSize cdsSize Name oname groupname),
   @HOKEY, qw( intron express location oid score scorevec) ); #f equiv1 equiv2 
# not for wasp: estgroup ; switch to tilegroup?
...
use constant { pSTRONG => 66, pMEDIUM => 33, pWEAK => 5, pTEWEAK => 19, pUTRPOOR => 33 };

# gequal.tab columns: use col header as REFtag ??
my $REF1tag = "ref1"; #f "nasviOGS1";  # CGDnnnn   //"APHIDBASE";  # ref1gene REF1tag = APHIDBASE
...
my $USE_TENAME= $ENV{tename} || $namecount; # FIXME?
...
if($namecount) { namecount($namecount); }
else { processtables(); }
...
sub processtables 
{
  ...
  elsif(/^gene_id\tname\t/) { $tb="n"; splice(@v,0,1); @hn=@v; push @hd, @v; } 
  elsif(/^gene_id\tclass/) { $tb="c"; splice(@v,0,1); @hc=@v; push @hd, @v; } 
  elsif(/^gene_id\tortholog\t/) { $tb="h"; splice(@v,0,1); @hh=@v; push @hd, @v; } 
  elsif(/^gene_id\txcover\t/) { $tb="x"; splice(@v,0,1); @hx=@v; push @hd, @v; } 
#f  elsif(/^gene_id\tref1gene\t/) { $tb="g"; splice(@v,0,1); @hg=@v; push @hd, @v; } 
  elsif(/^gene/i or /gene_id\t/i) { $tb="HUH?"; warn "# SKIPPING. Dont know table input: $_\n"; }
  
  elsif($tb eq "a") {
    my %v= vmap(\@ha,\@v); 
    map{ $v{$_}=0 if($v{$_} eq "na") } (qw(alttr tere nintron inqual));  
...
# Thecc1EG000001t1        249     76%,750/981
    ...
    my $scr=$v{score}; $scr=~s/,.*//; $$attr{score}= $scr; #buggy data
...
#     ($$attr{qualMated}  = $v{maqual}) =~ s,^.*/,, if $v{maqual};    # old
    ...
    if( $v{aalen} =~ /,/ ) { # ** FIXME: do after all tables, qualHomology not read yet?
      ...
      unless($aaq) { # BUG: missing this aalen annot for 1700 genes ..
        # better to fix this for input scores
        # ;oid=AUGepir1s302g51t1 Funhe5EG008673t1 aalen=687,65%; genes.gff
        ## better say $aaq= "unavailable" or other flag: quality=Protein:unavailable rather than Protein:None or Protein:missing 
        $aaq= "unavailable";
        #// $aaq= ($aal<40 or $aap<16)?"partial":"complete"; # bad default but need something;
      } 
    ...
    $$attr{chimera}=0; # change this to Split; retain old usage, rename Split
    if($v{chimera}) {
    ...
    if($cla =~ /^quality[:=]/) {
      ...
      # ==> pub11u-class.tab <==
      #Nasvi2EG000002t1 quality=Class:Strong,Express:Strong,Homology:OrthologStrong,Intron:Strong,
      ## attr.tbl
      # Class:Strong-quality:Class:Strong-expertchoice-Express:Strong-Homology:OrthologStrong-Intron:Strong-Protein:complete,Express:Weak
...
#     $$attr{estgroup}= $eg if($eg);
    ...
    ## add ggname  ggnamepct       ggnameref == orthogroup name
...
#f     $$attr{equiv1}= $na;   # print colname = REF1tag
#f     $$attr{equiv2}= $na2;  # print colname = REF2tag
        
  } elsif($tb eq "h") {
    my ($id,@v)= @v;
    my %v= vmap(\@hh,\@v); # ortholog,paralog,uniref now ?
    my $attr= getattr($id); 
    map{ $v{$_}="" if($v{$_} eq "na") } (@HOKEY); #f ,'Ncbiref');
    
    # FIXME: bitscores may have "."  (any e?)
    # $v{Dbxref} =~ s,^\w+/(\w+:),$1,; 
    #wasp: Dbxref=2102/2424,apis2ref:XP_623820.2 >> bit%,bits,ID
    # ncbi2ref: Ncbiref=tr:NM_001190685.1,pr:NP_001177614.1,GeneID:100463181 should add to Dbxref
#f    if($v{Dbxref}) { $v{Dbxref} =~ s/(\d+%),\d[^,]*,//; } # was (\d+) problems w/ this ho prefix
#f    if($v{Ncbiref}) { $v{Dbxref}.="," if($v{Dbxref}); $v{Dbxref} .= $v{Ncbiref}; }
...
#f    $$attr{uniprot}  = $v{uniprot};  # FIXME tag names
    ...
    map{ $$attr{$_} =~ s/(\d+%),\d[^,]*,(.+)/$2#$1/;  $$attr{$_} =~ s/,.*//; $$attr{$_} =~ s/#/,/; } 
         qw(ortholog paralog); ##f  uniprot # Dbxref?? losing ; but not genegroup
    
    if( $$attr{uniprot} and not $formatTABLE) { # merge Dbxref for .gff
      ## map{ $$attr{$_} =~ s=,=/=g; } qw(Dbxref uniprot);
      ## fixme: Not Ncbiref : Dbxref=NcbiRef2rna2392/tr:XM_001600948.2/pr:XP_001600998.2/GeneID:100116528
      map{ $$attr{$_} =~ s=,=/=g; } qw( uniprot);
    ...
    $pho=pWEAK if($pho=~/weak/i and $pho>pWEAK); # ortho/para flag from omcl-miss
    $pho =~ s/\D+//;
    $qho .= pqual($pho);
...
#       }
    ...
    ## xcover = _max($ec,$rc)
    ## $px= _max($px,$pasm); #? px NOT always >= pasm
    
    $$attr{express} = "$px\%,eq:$pasm"; ## or px,xcover
    
    #(my $nr= $v{rna_cover}) =~ s/^\d+,//;    
    #$$attr{express} = "$px\%,$nr";
    
    my $q= pqual( _max($px,$pasm) ); 
    # $q="Medium" if($q eq "Strong" and ($nr*70)/$trlen < 1.5);
    $$attr{qualExpress} = $q;
  }
...
sub nameclean {
  my($na,$hasid)= @_;
  #BEGIN{ $NAME_NONE  = "Unknown";  $NAME_NOFUN = "Uncharacterized protein"; }
  my ($isunk,$isput,$islike)= (0,0,0);
  my $NAME_UNK= $NAME_NONE; # alias
  my $id="";
  ...
  s/\bRefSeq\s+//i; # also RefSeq peptide XP_001942677
  ...
  s/,(EC|GN):[^\s,;]+//;  # ? drop/keep EC?
  #below# s/Isoform [\w-]+ of //i; s/\s*(isoform) [\w-]+//i; #|isozyme.*
  s/\s*transcript variant \w+//i;
  s/,\s*partial\s*\w*//;  
  #below# s/similar to\s*//i;
  ...
  s/Glean peptide GLEAN/GLEAN/i;
  s/Signal\-/Signal /i;
  #Slowpoke binding/Slowpoke-binding
  
  ## low quality protein:  prefix; who adds this?
  if(s/\blow quality protein:\s*//i) { } #? leave off? s/$/, low quality/; 
  ...
  #below# if(s/\-like//) { s/$/, putative/; }
  #below# if(/^(probable|putative)\s+\w+/i) { my $p=$1; s/^$p\s*//i; s/$/, putative/; }  
  unless(m/uncharacterized protein\s+\w+/i) { s/uncharacterized protein/$NAME_NONE/i; }
  #below# s/[,.]?\s+$//; s/^\s+//;

  $_= $NAME_NONE if( m/^(na|protein|unknown)$/i or ! m/\w/);

## insert/replace parts here from evigene/scripts/evigene2genbanktbl.pl:nameclean()  
##.........................
      # Fish cleans; #
  ...
  ## zebrafish crap names: Si:dkey-31m5.7; Si:ch211-225k7.3; Uncharacterized protein
  if(/^Si:/i) { $isunk=1; $_=$NAME_NONE; }
  ...
  s/\s\(Fragment\)//; 
  s/\s[Ii]soform\s*.*$//; #? leave in isoform xx ?
...
sub pqual { my $p=shift;  ## $p=pWEAK if($p=~/weak/i and $p>pWEAK); # ortho/para flag from omcl-miss
