...
sub idtype {
  ...
  my($sm)= $tid=~m/(EVm|Zm0|zeamjgi|ncbig|velv|soap|trin|idba|brid|binp|pacb)/;
  if($IDPRE) { ($sm)= substr($tid,0,$IDPRE); }
  ## corn pacbio id tags in xxx.aa: csh6earu:SRR3147027r8096; csh6tttt:SRRnnn
  if($tid =~ m/^csh6\w+/) { $sm="Cshpb6"; } # corn gramene pacbio
  if($tid =~ m/Arath(\dEVm)/) { $sm="At$1"; }
  if($tid =~ m/([cj]mix)/) { $sm=$1; } # arab6jmix .. special casee
  $sm="miss" unless($sm);
  $tt{$sm}++; 
  return ($sm);
}
