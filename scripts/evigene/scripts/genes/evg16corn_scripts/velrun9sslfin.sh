...
## velrun9sslfin.sh : oases finish for velrun9ssl.sh
...
kseqdir=vel${dv}_seq
#d if [ ! -f $kseqdir/Sequences ]; then
#d  $velbin/velveth $kseqdir 37 $vopth -noHash -shortPaired -fmtAuto -separate $inpe
#d fi
...
#d   velbin=$velbin4; 
#d   if [ $k -le 99 ]; then velbin=$velbin2; fi
#d   if [ $k -le 61 ]; then velbin=$velbin1; fi
#d   mkdir $ksubdir
