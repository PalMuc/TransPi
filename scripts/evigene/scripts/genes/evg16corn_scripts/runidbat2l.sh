...
# gordon.sdsc: ncpu=15; maxmem=50000; 
...
ov="o2"; traopts="--mink 31 --maxk 91 --step 10 ";

evigene=$HOME/bio/evigene/scripts
idbin=$HOME/bio/idba/bin
export PATH=$HOME/bio/idba/bin:$PATH
...
echo "#START  `date`"
echo "$idbin/idba_tran $traopts --long_read $inpe --num_threads $ncpu --out $outdir"
$idbin/idba_tran $traopts --long_read $inpe --num_threads $ncpu --out $outdir
echo "#DONE : `date`"
