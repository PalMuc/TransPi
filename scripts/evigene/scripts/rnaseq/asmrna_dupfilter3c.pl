...
my $debug= 0;
use constant VERSION =>  '2018.05.12'; # aaconsensus update
...
my($aconsensus, %aconsensus)= (undef); # AACONS
...
sub classifytr {
  ...
  my $aacons=0;
  if(AACONS) {
    $aacons= $aconsensus{$tid}||0;
    ## $ispoor= 0 if($aacons>0); # FIXME;
  }
...
if(CODEPOT1607) {
  ...
  if(AACONS) {
    # $aacons= $aconsensus{$tid}||0;
    # $ispoor= 0 if($aacons>0); # NO;
    if($aacons > 0) {
      $keepdrop .="okay:aacons$aacons,";
      $aadupflag .= ",aacons:$aacons";  #? remove aadup:mainid or not
      ## $ispoor & kAADUP : remove or not? NOT, has other uses/meaning
    }
  }

  # FIXME1805: rescue short/part cds with good/best match to refgenes
  # .. fragment/part class may include distinct loci, matching ref genes, but also w/ align to longer locus
  # .. prior  keepdrop= okay:refbest should keep; check for losses
...
# .. 48gaps/116aa is not good prot; add flag gapbad == kAAGAPS above in readAAqual ?
  ...
  ## AACONS: add flag here, like aadup:
...
### * add vQtlen before vTtlen, else get utrbad before good ***
  ...
  while(<$inh>) { # maybe local table, sorted, or from file
    ...
    #AACONS.2019 : special case of samesize  consensus(td) = rare (qd), swap  
    # NEEDS TEST .. if($aconsensus{$td} > $aconsensus{$qc}) .. swap qd,td
    
    $isfrag= $aclass="";
    $samesize=($tc == $qc)?1:0; # OR tinyalndiff? abs($tc - $qc) < $NHIALN
    my $swapqt=($tc > $qc)?1:0;
    if(AACONS) {
      $swapqt=1 if($nconsensus and $samesize and $aconsensus{$td} > $aconsensus{$qd});
    }
