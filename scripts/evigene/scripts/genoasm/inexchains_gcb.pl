...
use constant VERSION => '2016.08.27';# '2016.07.24'; # new, derived
...
my $INCHAINS=1; # not an option here; * REPURPOSE, turn off inchains output, for OUTBLTAB only
my $NOLOCUSID= 999999;
...
my $optok= GetOptions(
  ...
  "INCHAINS!", \$INCHAINS, # was fixed ON, now opposite of nobltab: noinchain implies outbltab only
...
if(1) {
  ...
  if(defined($OUTBLTAB)) {
    ...
    die "#ERR: bad option mix: -noinchain and -nobltab, need one or other\n";
  }
...
if(TESTOVERANY) {  
    ...
    if($TESTOVERALL) {
      ...
      if($isover) { 
        ...
        ## .. this may be too liberal at revoking intronchain call, only if what? above quals?
...
sub intableOutput { 
  ...
  return 0 unless($INCHAINS); # see above, only OUTBLTAB
