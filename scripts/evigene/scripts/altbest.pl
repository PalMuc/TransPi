...
my $CHANGEALTID=1; 
my ($maingenes, $altgenes, $eqtab, $altab, $debug, $dropnomain, $keepnointron, $keepids)= (0) x 9;
...
## eqtab evg4cornok2reb.main2alt.eqgene
$altab = $eqtab; 
$altab =~ s/\.equalalt//; $altab =~ s/\.eqgene//; $altab=~s/\.tab//; 
$altab.= ".altbest.tab";
...
my (%gmain,%galt,%ichain,%allalt2main,%allaltseen,%mainpcds,%gcds,%exonends);
...
my (%altnum, %didid, %galtok, %galtsame, %gmaindup, %gmaindupid, %alt2gid, %nclass, $ntabmain, $ntabalt);
...
if(TEST_ALTEND) {
    ...
    {  
      ...
      print ATAB join("\t","intrchain",$aid,$TAG_ALT.$anum,"$valclass,$inum,$debugval",$ichain)."\n"; # this is long table, changed cols.
   	} elsif(1 or $debug) {  # debug or always?
...
my($altid,$isalt,$isnewmain,$hasnomain,$galt,$gmain)= (0) x 9;
...
while(<G>) {
  ...
  if(/\tmRNA/) { 
    ...
    if($isnewmain) {
      ...
      if($altidc =~ /${TAG_ALT}1$/) { $altidc =~ s/${TAG_ALT}1$//; }  
      $altidc.= $TAG_ALT.$ai;
...
warn "# valid-alts put altvalid=$naltput, altnewloc=$nnewput, altnomain=$nnomainput, total=$natot\n" if $debug;
