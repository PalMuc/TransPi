#!/bin/bash
## env name=ztickfem inpe=sraf/SRR*_[12].fasta qsub -q normal velrun2.sh
#PBS -N velrun 
#PBS -A ind114
...
if [ "X" = "X$inpe" ]; then inpe=sraf/SRR*.fasta; fi
...
if [ ! -f $kseqdir/Sequences ]; then
 $velbin/velveth $kseqdir 27 -noHash \
    -fasta -shortPaired -separate $inpe  
fi
