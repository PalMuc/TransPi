...
=item about
 ...
 intron chain annotation:
   $evigene/scripts/overlapfilter -intron2splice=error -pass 'exon,intron' -act markid -midtype scoresum \
...
=item altbest3 1803
	- add intronok.tab use (evigene genes/intronoktab.pl), better suited to ichains
	$evigene/scripts/genes/intronoktab.pl -showerr -ValidANTIOFF -offby -intron $introngff $genes.gff \
		> $genes.intronok.tab
...
use constant VERSION => '2018.03.08'; # altbest v3
# use constant VERSION => '2016.07.20'; # altbest v2 ?

our $EVIGENES="$FindBin::Bin";  # $evigene="/bio/bio-grid/mb/evigene/scripts";
my $MINID_CDS=33; #was 66; -mincds=33
my $MINID_UTR=33; #was 40; -minexon=33   this is really full transcript span percent, utr+cds
my $MIN_PCDS=33; #was 40;  -mincoding=33  0=all, assume mRNA cxlen=cds,exon or calculate? use to filter falseutr models
my $MIN_CDS2MAIN= 0.20; #was 0.55; -minpcds=20  relative to main pcds, ie drop false long utr aberrations
my $MIN_ALTCHAINSIZE = 0.30; # -minintron=30  OPTION: min proportion of introns in short alt, of longest
...
my $ANTIOK=1; # antisense intron/cds valid?
my ($maingenes, $altgenes, $intronoktab, $eqtab, $outbase, $debug, $dropnomain, $skipnewloci, $keepnointron, $keepids, $dropids)= (0) x 9;
...
my $optok= GetOptions(
  ...
  "eqtab=s", \$eqtab, 
  "outbase=s", \$outbase, 
  "inoktab|intronoktab=s", \$intronoktab,  #1803 upd
  "mincds=i", \$MINID_CDS,  
...
my (%valinchain,%valin,%valerr); # readIntronOk intronchain/id, n valid/id, n invalid/id
# %valinchain replaces %ichain
...
warn "#options: -intronoktab genes.inok.tab -mincds=$MINID_CDS -minexon=$MINID_UTR -minintronchain=$MIN_ALTCHAINSIZE"
	." -mincoding=$MIN_PCDS -codingOfMain=$MIN_CDS2MAIN -tag=$TAG_ALT -keepids=keep.idlist\n" if $debug;
...
%valinchain=(); # replace %ichain
my($nin,$nval,$ntr)= ($intronoktab) ? readIntronOk($intronoktab) : (0); # upd1803
...
# see  evigene/scripts/genes/intronokfilter.pl : reader
# see  evigene/scripts/genes/intronoktab.pl : data source
sub readIntronOk {
	my($inf)= @_;
	my($nin,$nval,$ntr,$ltd)=(0) x 9;
	open(INH,$inf) or die "open $inf";
	while(<INH>) {
		next if(/^\W|^nogene/); 
		my @v=split;
		# intronok.tab: sorted by td > ind, geneid > inlocation
		my($td,$ind,$inw,$inv,$xtype)=@v; # xtype = CDS|UTR
		$xtype ||= "other"; # bug?
		$valinchain{$ltd}=~s/; $// if($ltd and $ltd ne $td);
		
		my($o)= substr($ind,-1,1); 
		my($wo)=($o eq '+')?'f':($o eq '-')?'r':'o';
		$ind= substr($ind,0,-1) . $wo;
		$ind=~s/\W/_/g; # make wordy
		
		my($vi)= ($inv =~ m/valintron=(\d+)/)?$1:0; 
		my($as)= ($inv =~ m/antisense=(\d+)/)?$1:0; 
		my($ler)= ($inv =~ m/longerr=(\d+)/)?$1:0; 
		...
		$iflag ||= "noevd";
		$nin++; $nval++ if($vi>0); 
		$ntr++ unless($td eq $ltd);
		
		# $tderr{$td}++ if($iflag=~/err/); 
		...
		# push @{$inchain{$td}}, $ind; # or 
	
		$valinchain{$td} .= "$ind; "; # revind?
		$valin{$td}++ if($vi>0);
		$valerr{$td}++ if($as>0 or $ler>0);
		# $valinflags{$td}{$ind}= join"\t",$iflag,$inw,$iv,$xtype;
		# my $val= ($vi>0) ? 1 : ($as>0 or $ler>0) ? -1 : 0;
		# $valin{$td}{0} += $val;
		# $valin{$td}{$ind}= $val; #??
	...
	warn "# readIntronOk nin=$nin, nvalid=$nval, ntr=$ntr, of $inf \n" if $debug;
	return($nin,$nval,$ntr);
}
...
# sort keepids{} first so others can drop?
	foreach my $gid (sort keys %galt) {
		...
		my @alt= @{$galt{$gid}};
		my $dn1= $valinchain{$gid} || $ichain{$gid}; # 1803upd; FIX here for no introns.
		
		## want some output for missing alts/chains?
		$galtok{$gid}= [] unless($galtok{$gid}); # for altsof end table
		next unless(@alt); #x next unless($dn1 and @alt);
			
		#o my @dns= sort{ length($ichain{$b}) <=> length($ichain{$a}) or $a cmp $b } grep{ $ichain{$_} } @alt;
		my @dns= grep{ $valinchain{$_} || $ichain{$_} } @alt;
		next unless(@dns);
		unless($dn1) { $dn1= $dns[0]; }	# first alt is best align to main
		
		## clean up warns "uninitialized value in length(noval)" ..
		# @dns= sort{ length($valinchain{$b}) <=> length($valinchain{$a}) 
		#	 or length($ichain{$b}) <=> length($ichain{$a})  or $a cmp $b } @dns;
			 
		my %dnlen= map{ my $dn= $valinchain{$_} || $ichain{$_}; my $l= $dn=~tr/;/;/;  $_ => $l } @dns;
	  @dns= sort{ $dnlen{$b} <=> $dnlen{$a} or $a cmp $b } @dns;
		...
		foreach my $aid ($gid, @dns) { 
			my $ichain= $valinchain{$aid} || $ichain{$aid} || "nochain"; # 1803upd;
	...
	if(TEST_ALTEND) {
			...
			if($icvalid<=0) {
				...
				if($aidi>1 and $aidi<9 and not $altsameloc) {
					...
					my $icvalo=0;
					my $ichaino= $valinchain{$gido} || $ichain{$gido}; # upd1803
					if($ichaino) {
...
sub newlociFromIchains # test 1607
{
  ...
  #  	my $altineqtab= $allaltseen{$gid} || $allaltseen{$gidc} || 0; ## $gid,$gidc
	...
	for my $aid (@testid) {
		# look for common subchains to collect new loci
		my $ichain= $valinchain{$aid} || $ichain{$aid}; # upd1803
		my ($nida,$icommon)= (0,0);
		my (%ins,%nids);
		my($aidXXX,$aidc,$asplit)=splitID($aid,"");
...
use constant ICVALID_NEWLOC => 1;	
	...
	for my $gidnew (sort keys %newlocids) {
		...
		#x @aid= sort{ length($ichain{$b}) <=> length($ichain{$a}) or $a cmp $b } @aid;
		@aid= sort{ length($valinchain{$b}) <=> length($valinchain{$a}) 
			or length($ichain{$b}) <=> length($ichain{$a}) or $a cmp $b } @aid;
  	...
  	unshift(@aid,$gmain) if($gmain);
		for my $aid (@aid) {
			...
			my $dni= $valinchain{$aid} || $ichain{$aid}||"noin"; # look for common subchains to collect new loci
			my $inum= $dni=~tr/;/;/; 
