...
my $SKIPSELF=0; # 2015 upd, skip overlaps of same id, same locusid (-tNNN), ..
our $ALTIDSUFFIX= 't\d+';
#  .. this is in overgenedup.pl but that is too complex, gene-overlap detector
...
my $optok= GetOptions(
  ...
  "selfskip!", \$SKIPSELF, #?and  "ALTIDSUFFIX=s", \$ALTIDSUFFIX,  # skip over self/alt ids
  "mark=s", \$mark, 
  "input|in=s", \$input,  
  "itype=s", \$itype,  
...
usage:  perl overlapfilter -act keep|drop|mark|markid|markbase -mark=terepeat 
        ...
        -selfskip : skip overlaps by ID same or alternate suffix '$ALTIDSUFFIX', needs midtype default
...
sub filter_gff
{
  ...
  while(<$inh>){
    ...
    ## 2015upd need IDs from tattr now..
    my $gid=0; 
...
## $intron2splice == kINTRONERROR_OVER << new
    ...
    my $isover= ($overtype == kOVERLAP) ? overlaps($ref,$tb,$te,$to,$gid)
              : ($overtype == kSAMELOC) ? sameloc($ref,$tb,$te,$to,$typ,$gid) 
              : ($overtype == kNEARLOC) ? nearloc($ref,$tb,$te,$gid) 
              : ($overtype == kCUTOVER) ? cut_overlaps($ref,$tb,$te,$to,$gid) 
              : overlaps($ref,$tb,$te,$to,$gid); # added to 2010.10

    if( ($intron2splice == kINTRON2SPLICE_QUERY 
      or $intron2splice == kINTRON2SPLICE_OVER)
      and $te2) {
      
      my $over2= ($overtype == kOVERLAP) ? overlaps($ref,$tb2,$te2,$to,$gid)
          : ($overtype == kSAMELOC) ? sameloc($ref,$tb2,$te2,$to,$typ,$gid) 
          : ($overtype == kNEARLOC) ? nearloc($ref,$tb2,$te2,$gid) 
          : ($overtype == kCUTOVER) ? cut_overlaps($ref,$tb2,$te2,$to,$gid) 
          : overlaps($ref,$tb2,$te2,$to,$gid); # added to 2010.10
...
sub overlaps
{
  my($ref,$tb,$te,$to,$tid)= @_;    
  ## FIXME: 15upd need input toid with tb,te,to ..
  (my $tgeneid=$tid) =~ s/${ALTIDSUFFIX}$//;
  my (@lid, @oids);
  $save_baseover= $save_pctover= 0;
  my $twidth= 1 + $te - $tb;
  ...
  for (my $ib = $ib1; $ib <= $ib2; $ib++) 
  {
    ...
    foreach my $rloc (@locs) {
      ...
      ## 2015upd $lid == ID/Parent of gff for default uses.. see markid opt tho
   ...
   ## 15upd: cancel self-matches by ID (oid?) and gene-locus-id (prefixid - tNNN)
      #?? bug in tid match?# $over=0 if($SKIPSELF and ($lid eq $tid or $lid =~ /^$tgeneid/)); # same gene..
      $over=0 if($SKIPSELF and index($lid,$tgeneid) == 0); # same gene..
...
#   $sum_pcttotal  += 1;
  ...
  for (my $ib = $ib1; $ib <= $ib2; $ib++) 
  {
   ...
   ## 15upd: cancel self-matches by ID (oid?) and gene-locus-id (prefixid - tNNN)
      $over=0 if($SKIPSELF and ($lid eq $tid or $lid =~ /^$tgeneid/)); # truly same gene..
...
sub nearloc
{
  ...
  for (my $ib = $ib1; $ib <= $ib2; $ib++) 
  {
    ...
    foreach my $rloc (@locs) {
     ...
     ## 15upd: cancel self-matches by ID (oid?) and gene-locus-id (prefixid - tNNN)
     ##?? not near?#  $over=0 if($SKIPSELF and ($lid eq $tid or $lid =~ /^$tgeneid/)); # truly same gene..
...
#  warn join",",("sameloc",$ref,$tb,$te,$to,$typ),"\n" if $debug and $warns++<10;
  ...
  foreach my $ib (@bins) {
   ...
   ## 15upd: cancel self-matches by ID (oid?) and gene-locus-id (prefixid - tNNN)
      next if($SKIPSELF and ($lid eq $tid or $lid =~ /^$tgeneid/)); # truly same gene..
