...
use constant VERSION =>  '2018.06.18'; # aaconsensus update: uncertain value; utrorf bugfix
...
sub nrcheckaaqual 
{
  ...
  my $dobett= ($idbest ne $dmain);
  my $addflag= "";
  if($USE_AACONS) {  # add header flag in better->{dmain}= newhdr
    if($nrtest=~m/,(agree=\w+)/) { $addflag=$1; $dobett=1; }
