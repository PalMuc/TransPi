...
if (params.all) {
    ...
    process trinity_assembly {
        ...
        output:
            ...
            tuple sample_id, file("${sample_id}.Trinity.fa") into busco_ch_trinity
    ...
    process busco {
        ...
        output:
            ...
            tuple sample_id, file("short_summary_${sample_id}.fa.bus.txt") into busco_summary
            tuple sample_id, file("short_summary_${sample_id}.fa.bus.txt") into busco_comp_1
    ...
    process busco_tri {
        ...
        output:
            tuple sample_id, file("short_summary_${sample_id}.Trinity.fa.bus.txt") into busco_ch_trinity_sum
            tuple sample_id, file("short_summary_${sample_id}.Trinity.fa.bus.txt") into busco_comp_2
    ...
    process get_busco_comparison {
        ...
        output:
            tuple sample_id, file("${sample_id}_combined_busco_figure.png") into busco_fig
    ...
    process get_GO_comparison {
        ...
        script:
            ...
            | cut -f 2 -d "^" | sort | uniq -c | sort -nr | head -n 15 | sed 's/^ *//g' | sed -r 's/[0-9] /\0#/g' | sed 's/ #/\t/g' >GO_cellular.txt

            cat ${sample_id}.trinotate_annotation_report.xls | cut -f 16 | tr "\\`" "\n" | grep "GO:" | cut -f 2- -d "^" | tr [a-z] [A-Z] | grep "BIOLOGICAL_PROCESS" \
            | cut -f 2 -d "^" | sort | uniq -c | sort -nr | head -n 15 | sed 's/^ *//g' | sed -r 's/[0-9] /\0#/g' | sed 's/ #/\t/g' >GO_biological.txt

            cat ${sample_id}.trinotate_annotation_report.xls | cut -f 16 | tr "\\`" "\n" | grep "GO:" | cut -f 2- -d "^" | tr [a-z] [A-Z] | grep "MOLECULAR_FUNCTION" \
            | cut -f 2 -d "^" | sort | uniq -c | sort -nr | head -n 15 | sed 's/^ *//g' | sed -r 's/[0-9] /\0#/g' | sed 's/ #/\t/g' >GO_molecular.txt
