...
```{bash cat_busco3_dist_mis, echo=FALSE}
cat *_missing_BUSCO3_table.tsv | sed -e 's/Missing/0/g' -e 's/Fragmented/1/g' -e 's/Duplicated/2/g' -e 's/Complete/3/g' >BUSCO3_missing_table.tsv

```
```{r busco3_dist_mis, echo=FALSE}
if (params$buscoDist == "true" && params$allBuscos == "true") {
    writeLines("\t\t\t\tHEATMAP")
    csv=read.csv("BUSCO3_missing_table.tsv", header=TRUE, sep="\t")
    ...
    fig <- fig %>% config(toImageButtonOptions=list(format='svg',filename='busco3_TransPi_missing_distribution_plot', height= 500, width= 800, scale= 1), displaylogo = FALSE)
    fig
} else {
    writeLines("\n--------------------------------------------------------------\n")
    cat(readLines(list.files(pattern="busco3_dist.txt")) , sep = '\n')
    writeLines("\n--------------------------------------------------------------\n")
}
```
...
**Missing BUSCO distribution**
```{bash cat_busco4_dist_mis, echo=FALSE}
cat *_missing_BUSCO4_table.tsv | sed -e 's/Missing/0/g' -e 's/Fragmented/1/g' -e 's/Duplicated/2/g' -e 's/Complete/3/g' >BUSCO4_missing_table.tsv
