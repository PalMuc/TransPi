...
params:
  interactive: yes
  data: !r commandArgs(trailingOnly=T)[2]
---
...
```{bash reads_stats, include=FALSE}
cat *_reads_stats.csv >final_reads_stats.csv
```
...
```{bash reads_qual, include=FALSE}
cat *_reads_qual.csv >final_reads_qual.csv
```
```{r qual_plot, echo=FALSE}
rqual=read.csv("final_reads_qual.csv",header=FALSE)
...
```{bash pre_EG, include=FALSE}
cat *_preEG.csv >preEG.csv
```
```{r pre_EG_table, echo=FALSE}
trans_preEG=read.csv("preEG.csv")
...
```{bash EG, include=FALSE}
cat *_EG.csv >EG.csv
```
```{r EG_table, echo=FALSE, results='asis'}
trans_EG=read.csv("EG.csv")
...
peg <- plot_ly(eg, x = ~V1, y = ~V2, type = 'bar', name = 'Before', marker=list(color='#088da5'), opacity=0.8, hovertemplate = paste('Program: %{x}','<br>Number of transcripts: %{y}','<extra></extra>'))
...
```{bash trans_len, include=FALSE}
cat *_sizes.txt >final_sizes.txt
```
```{r trans_len_plot, echo=FALSE, results='asis'}
tlen=read.delim("final_sizes.txt", sep = "\t", header = FALSE)
...
bus=read.csv("final_busco3.csv", header=FALSE)
...
bus=read.csv("final_busco4.csv",header=FALSE)
MYSPEC=as.character(t(bus[1,]))
MYPERC=as.numeric(t(bus[2,]))
MYVAL=as.numeric(t(bus[3,]))
######################################
...
dataCC=read.delim("final_GO_cellular.csv", header = F, sep = "\t")
...
dataMF=read.delim("final_GO_molecular.csv", header = F, sep = "\t")
...
dataBP=read.delim("final_GO_biological.csv", header = F, sep = "\t")
...
cat *_custom_uniprot_hits.txt >final_custom_uniprot_hits.csv
```
