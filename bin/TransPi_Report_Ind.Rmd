...
if (params$skipFilter == "true") {
    writeLines("\n--------------------------------------------------------------\n")
    writeLines("\t\t\t\tFiltering process was skipped")
    writeLines("\n--------------------------------------------------------------\n")
} else {
...
if (params$rRNAfilter == "true") {
    writeLines("\t\t\t\tPLOT HERE")
} else {
    writeLines("\n--------------------------------------------------------------\n")
    writeLines("\t\t\t\tNo rRNA filtration was performed")
    writeLines("\n--------------------------------------------------------------\n")
}
```
<br>
...
if (params$skipNormalization == "true") {
    writeLines("\n--------------------------------------------------------------\n")
    writeLines("\t\t\t\tNormalization process was skipped")
    writeLines("\n--------------------------------------------------------------\n")
} else {
    writeLines("\t\t\t\tPLOT HERE")
}
...
trans_preEG=read.csv(paste(params$sample_id,sep="",".sum_preEG.csv"))
...
trans_EG=read.csv(paste(params$sample_id,sep="",".sum_EG.csv"))
...
**Mapping stats**
```{r mapping_stats, echo=FALSE, results='asis'}
#mapping_trinity
#mapping_evigene
```
...
```{bash busco3, echo=FALSE}
cat *_busco3.csv >final_busco3.csv
```
```{r busco3_plot, echo=FALSE}
bus=read.csv(paste(params$sample_id,sep="","_busco3.csv"), header=FALSE)
...
bp <- bp %>% config(toImageButtonOptions=list(format='svg',filename='busco3_plot', height= 500, width= 800, scale= 1), displaylogo = FALSE)
bp
```
...
```{bash busco4, echo=FALSE}
cat *_busco4.csv >final_busco4.csv
```
...
```{r busco_dist, echo=FALSE}
if (params$buscoDist == "true" && params$allBuscos == "true") {
    writeLines("\t\t\t\tHEATMAP")
    csv=read.csv(paste(params$sample_id,sep="","_TransPi_comparison_table.tsv"), header=TRUE, sep="\t")
    #csv=read.csv("SRR10527303_Octolasmis_warwickii_complete_comparison_table.tsv", header=TRUE, sep="\t")
    ...
    writeLines("\n--------------------------------------------------------------\n")
    writeLines("\t\t\t\tBUSCO distribution analysis was skipped")
    writeLines("\n--------------------------------------------------------------\n")
}
```
...
```{bash transdecoder, include=FALSE}
cat *.transdecoder.csv >final_transdecoder.csv
```
```{r trandecoder_table, echo=FALSE, results='asis'}
paged_table(read.csv("final_transdecoder.csv"),options = list(rows.print = 10))
```
...
<br>
```{bash go1, echo=FALSE}
cat *_cellular.csv >final_GO_cellular.csv
```
```{r go_plots1, echo=FALSE}
#go_sample_name=params$data
dataCC=read.csv("final_GO_cellular.csv", header = F)
...
<br>
```{bash go2, echo=FALSE}
cat *_molecular.csv >final_GO_molecular.csv
```
```{r go_plots2, echo=FALSE}
dataMF=read.csv("final_GO_molecular.csv", header = F)
...
<br>
```{bash go3, echo=FALSE}
cat *_biological.csv >final_GO_biological.csv
```
```{r go_plots3, echo=FALSE}
dataBP=read.csv("final_GO_biological.csv", header = F)
...
```{bash custom_uniprot, echo=FALSE}
cat *_custom_uniprot_hits.csv >final_custom_uniprot_hits.csv
```
```{r custom_uniprot_plot, echo=FALSE}
dataUni=read.csv("final_custom_uniprot_hits.csv", header=F)
